...
void InnerEmbedEBUCoreMetadata(	
		...
		if (optWaytoWrite != DARK && optWaytoWrite != NONE) {
			...
			if (optWaytoWrite == SIDECAR) {
				framework = EBUCore::GenerateSideCarFramework(metadataLocation, &*mHeaderMetadata, id);
			} else {
				...
				framework = EBUCore::Process(ebuCoreMainElementPtr, metadataLocation, &*mHeaderMetadata, eventFrameworks, id);
			}
